# Документация LLM Gateway

| Документ | Аудитория | Описание |
|----------|-----------|----------|
| [client_integration_guide.md](client_integration_guide.md) | Интеграторы | API, SDK, примеры, аутентификация, все фичи |
| [internal_logic.md](internal_logic.md) | Разработчики | Архитектура, компоненты, процедуры обновления |
| [operational_runbook.md](operational_runbook.md) | On-call | Debug, инциденты, мониторинг, эскалация |
| [microservices_setup_guide.md](microservices_setup_guide.md) | DevOps/SRE | Deployment, Docker, ENV, sizing |
| [commands.md](commands.md) | Все | CLI-команды с описанием и синтаксисом |
| [raw_response_specification.md](raw_response_specification.md) | Разработчики | Схема хранения сырых данных, retention, PII |

Общее описание проекта: [../CLAUDE.md](../CLAUDE.md)
